...
대시보드는 **좌측 사이드바의 두 탭**으로 나뉜다 — **설정**(연결/작업공간 경계/카메라/
스키마/안전 한계, 전부 준비 단계) / **배포**(감지·확정·Start/Stop/E-STOP·텔레메트리,
실제로 돌리는 단계). 탭은 브라우저에 기억되고(새로고침해도 마지막 탭 유지), URL에
`?tab=deploy`처럼 붙여서 바로 열 수도 있다.
...
작업공간 경계 패널 참고). 재시작 후엔 설정을 다시 입력하고 **반드시 "설정 저장" 버튼을
눌러야** 서버에 반영된다.
...
![설정 탭](screenshots/dashboard-full.png)
...
- **먼저 연결 패널에서 "설정 저장"을 눌러야** 여기서 카메라를 열 수 있다 (서버가 어떤
...
- **텍스트 에디터**: 위 폼으로 만든 JSON이 그대로 여기 나타나고, 직접 편집도 가능
  (둘이 서로 실시간 반영됨). 다 되면 **"스키마 적용"** 클릭 → 연결 패널의 "설정 저장"과
  함께 최종 저장됨.
...
![안전 한계 패널](screenshots/panel-5-safety.png)
...
곳으로 가는" 문제는 위 작업공간 경계 패널이 담당 — 둘은 서로 다른 종류의 문제라
따로 뒀다.
...
![배포 탭](screenshots/dashboard-deploy.png)

## 응답 포맷 감지 & 확인 패널

![감지 패널](screenshots/panel-6-detect.png)
...
- **reset instruction**: `/reset`에 실어 보낼 지시문 (연결 패널에서 instruction
  필드명을 설정했을 때만 의미 있음).
...
2. **설정 탭**: 연결 값 입력 → 스키마 작성 → 안전 한계(보통 기본값) →
   **"설정 저장" 클릭 (제일 중요, 자주 빼먹음)**
3. `read_only` 체크된 채로 **연결**
4. (권장, 새 환경/새 정책 처음 테스트할 때) 작업공간 경계 패널에서 Programming
   모드로 전환 후 안전 범위 경계점 2개 이상 기록
5. (선택) 카메라 확인
6. **배포 탭**으로 이동: **감지 실행** → 결과 확인/필요시 직접 수정 → **이대로 확정**
