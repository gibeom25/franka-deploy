...
franka-deploy는 **로봇 제어와 웹서버를 서로 다른 프로세스**로 분리해서 돌린다 (IK 계산 같은
무거운 파이썬 작업이 로봇의 1kHz 제어 스레드와 같은 GIL을 공유하면 실제로 리플렉스
크래시가 났던 적이 있어서 — `README.md`의 "Architecture" 섹션 참고).
...
있고 디스크에 저장되지 않기 때문. 재시작 후엔 설정을 다시 입력하고 **반드시 "설정
저장" 버튼을 눌러야** 서버에 반영된다 (아래 1번 패널 참고).
...
| **설정 저장** | **여기 입력한 모든 값(연결 정보 + 아래 스키마 + 안전 한계까지 전부)을 서버에 실제로 반영하는 버튼.** 이거 안 누르면 드롭다운/입력칸에 뭘 골라도 서버는 모른다 — 가장 자주 하는 실수. |
...
![카메라 확인 패널](screenshots/panel-2-cameras.png)
...
![송신 스키마 패널](screenshots/panel-3-schema.png)
...
![감지 패널](screenshots/panel-4-detect.png)
...
![실행 패널](screenshots/panel-6-run.png)
...
![텔레메트리 패널](screenshots/panel-7-telemetry.png)

WebSocket으로 0.1초마다 갱신되는 실시간 상태. `state`(현재 세션 상태), `q`/`dq`(관절
위치·속도), `ee_pos`, `control_command_success_rate`(1.0이 정상), `last_predict_ms`
(정책 서버 응답 지연), `n_replans`/`n_late`(재계획 횟수/예산 초과 횟수), `error`(있으면
크래시 원인 메시지)가 전부 여기 보인다. 문제 생기면 제일 먼저 볼 곳.

---

## 전체 실행 순서 요약

1. 로봇 노드 + 앱 서버 실행 (위 "실행 방법")
2. 대시보드 접속 → 1번 패널 값 입력 → 2번(스키마) 작성 → 5번(안전 한계, 보통 기본값) →
   **"설정 저장" 클릭 (제일 중요, 자주 빼먹음)**
3. `read_only` 체크된 채로 **연결**
4. (선택) 카메라 확인
5. **감지 실행** → 결과 확인/필요시 직접 수정 → **이대로 확정**
6. **Start** — 실제 모션 시작. 문제 생기면 Stop 또는 E-STOP.
7. 설정을 다시 바꾸려면 **연결 해제**부터.
...
| `call /connect first` | 연결을 안 한 채로 감지를 눌렀음 |
