...
대시보드는 **좌측 사이드바의 두 탭**으로 나뉜다 — **설정**(연결/작업공간 경계/스키마/
안전 한계, 전부 준비 단계) / **배포**(카메라 확인/감지·확정/Start·Stop·E-STOP/텔레메트리,
실제로 돌리는 단계). 설정 탭 안에는 다시 **상단 서브탭 4개**(연결/작업공간 경계/송신
스키마/안전 한계)가 있어 패널 하나씩만 보인다. 탭/서브탭 선택은 브라우저에 기억되고
(새로고침해도 유지), URL에 `?tab=deploy` 또는 `?tab=settings&subtab=schema`처럼
붙여서 바로 열 수도 있다.

**입력한 설정은 이제 디스크에 저장된다** (`~/.franka-deploy/config.json`,
`workspace.json`) — 앱 서버를 재시작해도 안 지워짐. "설정 저장"을 누르는 순간, 또는
작업공간 경계점을 추가/삭제/margin 변경하는 순간 바로 저장된다.
...
![연결 패널](screenshots/panel-connection.png)
...
| agentview / eye_in_hand 카메라 | 드롭다운 — 페이지 열면 자동으로 연결된 RealSense 카메라를 스캔해서 보여줌. 시리얼 고르면 그 카메라를 쓰고, `(mock 이미지)`를 고르면 카메라 없이 랜덤 이미지로 대체(하드웨어 없이 테스트할 때 유용). 여기서 정한 role(agentview/eye_in_hand)을 송신 스키마의 `camera:agentview` 같은 출처가 그대로 참조한다 — 스키마 쪽엔 따로 설정할 게 없음. "카메라 검색" 버튼으로 다시 스캔 가능. |
| **설정 저장** | **여기 입력한 모든 값(연결 정보 + 스키마 + 안전 한계까지 전부)을 서버에 반영 + 디스크에 저장하는 버튼.** 이거 안 누르면 드롭다운/입력칸에 뭘 골라도 서버는 모른다 — 가장 자주 하는 실수. |
...
## 작업공간 경계 서브탭 (teleop으로 설정)

![작업공간 경계 패널](screenshots/panel-workspace.png)

관절 리밋이나 레퍼런스 필터와는 별개의, **마지막 안전장치**. "말도 안 되는 움직임"
(관절 제한 안, 부드럽긴 한데 엉뚱한 곳으로 가는 경우)을 막으려고, EE가 실제로 가도
되는 범위(**EE 기준 직육면체**)를 사람이 직접 정한다.

1. `read_only`로 **연결** (모션 명령 없이 상태만 읽으면 되므로 이걸로 충분)
2. Desk 웹 화면에서 로봇을 **Programming(흰색) 모드**로 전환 — Execution/파란색
   모드에선 손으로 못 움직인다. (franka-deploy에는 이 모드를 소프트웨어로 바꾸는
   버튼이 없다 — 설치된 pylibfranka 바인딩에 guiding-mode 제어 함수 자체가 없고,
   애초에 Desk 쪽 물리 안전 스위치라 Desk에서 직접 전환해야 함.)
3. **enabling 버튼을 누른 채로** 로봇을 손으로 잡고 원하는 안전 범위의 한쪽 모서리로
   이동 → **"현재 위치를 경계점으로 추가"** 클릭
...
이 경계점들은 **디스크에 저장돼 서버 재시작에도 살아남고**, `/api/config`나 연결
해제와도 무관하게 유지된다 — 로봇 팔 주변 환경은 세션이 아니라 이 물리적 설치에 대한
...
![송신 스키마 패널](screenshots/panel-schema.png)
...
- **"+ 필드 추가"**: JSON을 직접 안 쳐도 필드를 추가할 수 있는 폼.
  - **출처(source)**: 이 값이 어디서 오는지. `camera:agentview`, `camera:eye_in_hand`
    (연결 서브탭에서 정한 카메라 프레임 — 여기선 role 이름만 참조, 시리얼 설정은
    연결 서브탭에서 함) / `robot:joint_positions`, `robot:ee_pos`, `robot:ee_pos_quat`
    등 (현재 로봇 상태) / `static:아무값` (고정 텍스트/값) / `custom:user_plugins.함수명`
...
- **텍스트 에디터**: 위 폼으로 만든 JSON이 그대로 여기 나타나고, 직접 편집도 가능
  (둘이 서로 실시간 반영됨). 다 되면 **"스키마 적용"** 클릭 → 연결 서브탭의 "설정
  저장"과 함께 최종 저장됨.
...
![안전 한계 패널](screenshots/panel-safety.png)
...
곳으로 가는" 문제는 작업공간 경계 서브탭이 담당 — 둘은 서로 다른 종류의 문제라
따로 뒀다.
...
![카메라 확인 패널](screenshots/panel-cameras.png)
...
- **먼저 설정 탭 → 연결 서브탭에서 "설정 저장"을 눌러야** 여기서 카메라를 열 수 있다
  (서버가 어떤 카메라를 열지 모르면 시작 자체가 실패함).
...
![감지 패널](screenshots/panel-detect.png)
...
- **reset instruction**: `/reset`에 실어 보낼 지시문 (연결 서브탭에서 instruction
  필드명을 설정했을 때만 의미 있음).
...
![실행 패널](screenshots/panel-run.png)
...
![텔레메트리 패널](screenshots/panel-telemetry.png)
...
2. **설정 탭**: 연결 서브탭 값 입력 → 스키마 서브탭 작성 → 안전 한계(보통 기본값) →
   **"설정 저장" 클릭 (제일 중요, 자주 빼먹음)** — 이제 디스크에 저장되므로 다음부턴
   서버 재시작해도 다시 안 해도 됨.
3. `read_only` 체크된 채로 **연결**
4. (권장, 새 환경/새 정책 처음 테스트할 때) 작업공간 경계 서브탭에서 Programming
   모드로 전환 후 안전 범위 경계점 2개 이상 기록
5. **배포 탭**으로 이동: (선택) 카메라 확인 → **감지 실행** → 결과 확인/필요시 직접
   수정 → **이대로 확정**
6. **Start** — 실제 모션 시작. 문제 생기면 Stop 또는 E-STOP.
7. 설정을 다시 바꾸려면 **연결 해제**부터 (작업공간 경계점은 안 지워짐).
